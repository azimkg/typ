...
import cls from './standartLesson.module.css';


const letters = ['sad', 'asd', 'asd', 'a'];
const combinedString = letters.join('\n');
const StandartExersice: React.FC = () => {
          ...
          return (
            <div key={lineIndex} className={classes(cls.line)} >
              ...
              {lineIndex !== letters.length - 1 && (
                <>
                  <Enter key={`icon_${iconIndex}`} className={classes({ [cls.icon]: currentIndex < cumulativeIndex + 1 }, { [cls.errorEnter]: errorIndexes.includes(cumulativeIndex - 1) })} />
...
export default StandartExersice;
